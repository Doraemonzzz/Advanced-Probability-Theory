...
因为显然有$\lambda(\Pi) \subset \sigma(\Pi)$，所以只需证$\sigma(\Pi)\subset\lambda(\Pi)  $，从而只需证$\lambda(\Pi)$为$\sigma$代数，又因为$\lambda(\Pi)$为$\lambda $系，所以只要证明$\lambda(\Pi)$关于可列并封闭即可。取$A_n \in \lambda(\Pi)$，那么
$$
